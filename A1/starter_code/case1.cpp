...
void GHC_with_RRS(int* mapping,int z,int l,int** T,int** N,vector<int>&cs){
    bool changemap=false;
    vector<vector<bool>> visited(z,vector<bool>(l,false));
    while(true){
...
void SportsLayout::compute_allocation()
{
    ...
    for(int i=0;i<restart;i++){
        ...
        GHC_with_RRS(mapping,z,l,T,N,init_state);
        if(fcost>costmin){
    ...
    for(int i=0;i<z;i++)cout<<mapping[i]<<" ";cout<<endl;
}
